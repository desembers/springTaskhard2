...
public class AuthService {
    ...
    @Transactional
    public SignupResponse2 signup2(SignupRequest2 request) {
        ...
        return new SignupResponse2(bearerToken);
    }
    ...
    public SiginResponse2 signin2(SigninRequest2 request) {

        User user = userRepository.findByEmail(request.getEmail()).orElseThrow(
                () -> new IllegalArgumentException("가입되지 않는유저입니다.")
        );
        ...
        String bearerToken = jwtUtil.createToken(user.getId(), user.getEmail(), user.getNickname(), user.getUserRole());
        return new SiginResponse2(bearerToken);
    }
